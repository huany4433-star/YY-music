...
struct SearchView: View {
        ...
        return Button {
            ...
            HStack(spacing: 7) {
                if top3 {
                    ...
                    Text("\(index + 1)")
                        .font(YYMusicFont.appFont(11, .bold, .rounded))
                        .foregroundStyle(Color.beansComment)
                        .frame(width: 18, height: 18)
                }
                Text(word)
                    .font(YYMusicFont.appFont(top3 ? 15 : 14, top3 ? .bold : .medium))
    ...
    private func startSearch(_ text: String) async {
        ...
        searchTask = Task {
            searching = true
            errorMessage = nil
            YYMusicLogger.shared.log("搜索：\(provider.rawValue) [\(resultType.rawValue)] \(trimmed)", level: .info)
            ...
            do {
                ...
                case (.netease, .song):
                    ...
                    guard !Task.isCancelled else { return }
                    songResults = songs
                    if !songs.isEmpty { YYMusicHaptics.success() }
                case (.netease, .artist):
                ...
                YYMusicLogger.shared.log("搜索完成：\(provider.rawValue) [\(resultType.rawValue)] \(trimmed) 结果=\(count)", level: .info)
            } catch {
                guard !Task.isCancelled else { return }
                errorMessage = error.localizedDescription
                YYMusicLogger.shared.log("搜索失败：\(provider.rawValue) \(trimmed) - \(error.localizedDescription)", level: .error)
            }
        }
...
struct SearchTextField: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> UITextField {
        ...
        field.font = YYMusicFont.appUIFont(15)
    ...
    func updateUIView(_ uiView: UITextField, context: Context) {
        ...
        uiView.font = YYMusicFont.appUIFont(15)
