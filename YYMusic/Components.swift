...
struct YYMusicGlass<S: Shape>: View {
...
struct YYMusicSymbolReplace: ViewModifier {
    @ViewBuilder
...
struct YYMusicNavigationStack<Content: View>: View {
    @ViewBuilder var content: () -> Content
...
struct YYMusicSheetModifier: ViewModifier {
    let detents: [YYMusicDetent]
    var dragIndicator: Bool?

    @available(iOS 16, *)
    private static func makeDetents(_ detents: [YYMusicDetent]) -> Set<PresentationDetent> {
...
struct CoverImage: View {
    ...
    private var placeholderIcon: some View {
        ZStack {
            ...
            if let emptyHint {
                Text(emptyHint)
                    .font(YYMusicFont.appFont(max(11, min(size * 0.09, 15)), .medium))
...
struct SectionHeader: View {
    ...
    var body: some View {
        HStack(alignment: .firstTextBaseline, spacing: 8) {
            Text(title)
                .font(YYMusicFont.appFont(21, .bold))
