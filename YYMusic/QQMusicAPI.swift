...
final class QQMusicAPI {
    ...
    private func resolveMediaMid(songmid: String, provided: String?, qqAuth: QQMusicAuth) async -> String {
        ...
        guard let json = try? await musicu(payload, cookie: qqAuth.isLoggedIn ? qqAuth.cookieHeader : ""),
            ...
            YYMusicLogger.shared.log("QQ 歌曲详情未返回 media_mid，回退 songmid", level: .debug)
            return songmid
        }
        YYMusicLogger.shared.log("QQ 歌曲详情：media_mid=已获取 是否不同=\(mediaMid == songmid ? "否" : "是")", level: .debug)
        return mediaMid
    }
    ...
    private func vkeyURL(songmid: String, mediaMid: String?, br: String, uin: String, loginKey: String, guid: String, qqAuth: QQMusicAuth) async throws -> String? {
        ...
        guard !playableInfos.isEmpty else {
            ...
            YYMusicLogger.shared.log("QQ vkey 无可播地址：音质=\(br) result=\(result) 已登录=\(qqAuth.isLoggedIn ? "是" : "否")", level: .debug)
            return nil
        }
        ...
        for info in playableInfos {
            ...
            for candidate in candidateURLs {
                ...
                if await probeAudioURL(url, cookie: qqAuth.isLoggedIn ? qqAuth.cookieHeader : "") {
                    ...
                    YYMusicLogger.shared.log("QQ 音频地址验证成功：音质=\(br) 文件=\(filename)", level: .debug)
                    return candidate
                }
            }
        }
        YYMusicLogger.shared.log("QQ vkey 返回地址但 CDN 验证失败：音质=\(br) 候选=\(playableInfos.count)", level: .debug)
        return nil
    }
