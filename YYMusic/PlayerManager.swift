...
final class PlayerManager: NSObject, ObservableObject {
    ...
    private func loadCurrent() {
        ...
        Task {
            ...
            let quality = YYMusicAudioQuality.current
            YYMusicLogger.shared.log("▶ 开始播放：\(song.name) - \(song.artists)｜平台=\(song.source.rawValue) id=\(song.id) 音质=\(quality.level) 免费听歌=\(enableUnblock ? "开" : "关") 官方受限=\(strictUnlock ? "是" : "否")", level: .info)
            ...
            guard let urlString, let url = URL(string: urlString) else {
                ...
                await MainActor.run {
                    ...
                    if song.source != .kugou, self.shouldLockOfficialOnly(song) {
                        YYMusicLogger.shared.log("播放失败：\(song.name) - 未找到原唱音源（官方受限），拒绝翻唱版本", level: .error)
                        ToastCenter.shared.show("《\(song.name)》未找到原唱音源（官方受限），已停止播放，拒绝翻唱版本")
                    } else {
                        let hint = thirdPartyAttempted ? "（第三方音源尝试后无结果）" : "（第三方音源未命中）"
                        YYMusicLogger.shared.log("播放失败：\(song.name) - 无法解析播放地址\(hint)｜音质=\(quality.level) 免费听歌=\(enableUnblock ? "开" : "关")", level: .error)
                    }
                }
                return
            }
    ...
    private func neteaseResolve(song: Song, quality: YYMusicAudioQuality, enableUnblock: Bool, strict: Bool = false) async -> (String?, UnblockService.Resolved?) {
        var urlString: String?
        ...
        YYMusicLogger.shared.log("网易云解析：\(song.name) 音质=\(quality.level) 官方URL=\(info?.url == nil ? "无" : "有") 试听=\(info?.freeTrial == true ? "是" : "否")", level: .debug)
        ...
        YYMusicLogger.shared.log("网易云结果：\(song.name) 官方=\(urlString != nil ? "是" : "否") 第三方=\(resolved != nil ? "命中" : "未用/未命中")", level: .debug)
        return (urlString, resolved)
    }

    /// QQ 歌曲兜底：先在网易云按 歌名+歌手 匹配同名歌曲，免费完整 URL 直接播，VIP/无 URL 交给第三方解锁
    private func qqFallback(song: Song, quality: YYMusicAudioQuality, enableUnblock: Bool, strict: Bool = false) async -> (String?, UnblockService.Resolved?) {
        var urlString: String?
        var resolved: UnblockService.Resolved?
        if enableUnblock {
            resolved = await UnblockService.resolve(
        ...
        YYMusicLogger.shared.log("QQ兜底：\(song.name) 官方=\(urlString != nil ? "是" : "否") 第三方=\(resolved != nil ? "命中" : "未用/未命中")", level: .debug)
        return (urlString, resolved)
    }
    ...
    private func kugouFallback(song: Song, enableUnblock: Bool) async -> UnblockService.Resolved? {
        ...
        if kugouID.isEmpty {
            YYMusicLogger.shared.log("酷狗兜底跳过：缺少 album_audio_id/hash", level: .debug)
        } else {
            ...
            if let resolved {
                YYMusicLogger.shared.log("酷狗兜底：\(song.name) 酷狗音源=命中", level: .debug)
                return resolved
            }
        }
        ...
        if let matched = await matchNetEaseSong(
            ...
            YYMusicLogger.shared.log("酷狗兜底转网易云音源：\(song.name) -> \(matched.name) 第三方=\(resolved != nil ? "命中" : "未命中")", level: .debug)
            return resolved
        }

        YYMusicLogger.shared.log("酷狗兜底：\(song.name) 第三方=未命中", level: .debug)
        return nil
    }
    ...
    private func setupPlayer(url: URL, thirdPartyVIPNotice: ThirdPartyVIPNotice? = nil) {
        ...
        itemStatusObserver = item.observe(\.status, options: [.new]) { [weak self] item, _ in
            ...
            YYMusicLogger.shared.log("播放地址加载失败：\(item.error?.localizedDescription ?? "未知错误")", level: .error)
        }
        timeControlStatusObserver = player.observe(\.timeControlStatus, options: [.new]) { [weak self] player, _ in
            ...
            if let song = self.currentSong {
                YYMusicLogger.shared.log("▶ 播放成功：\(song.name)｜域名=\(url.host ?? "?")", level: .info)
            }
        ...
        failureObserver = NotificationCenter.default.addObserver(forName: .AVPlayerItemFailedToPlayToEndTime, object: item, queue: .main) { [weak self] _ in
            self?.loadFailed = true
            self?.isBuffering = false
            YYMusicLogger.shared.log("播放中断：AVPlayerItem 播放失败（解码或网络错误）", level: .error)
        }
        updateNowPlaying()
    }
    ...
    private func showPendingThirdPartyVIPNoticeIfNeeded() {
        ...
        YYMusicLogger.shared.log("第三方音源会员歌提醒：\(notice.message)", level: .info)
        pendingThirdPartyVIPNotice = nil
    }
    ...
    static func applyAudioMixPreference(_ mixesWithOthers: Bool) -> Bool {
        do {
            ...
            YYMusicLogger.shared.log("音频会话配置失败：\(error.localizedDescription)", level: .error)
            return false
        }
    }
    ...
    /// 与其他 App 音频混合播放。默认关闭，让系统把 YYMusic 作为主播放 App 显示到锁屏/灵动岛。
