...
struct ProfileView: View {
    ...
    private func wallpaperCell(path: String) -> some View {
        ...
        return ZStack(alignment: .topTrailing) {
            Button {
                ...
                Group {
                    if let img = YYMusicImageFileCache.image(at: path) {
                        Image(uiImage: img)
                            .resizable()
                            .scaledToFill()
                    } else {
    ...
    private func featureCell(icon: String, title: String, subtitle: String, action: @escaping () -> Void) -> some View {
        Button {
            ...
            .padding(12)
            .background {
                                YYMusicGlass(shape: RoundedRectangle(cornerRadius: 18, style: .continuous))
            }
            .contentShape(Rectangle())
        }
...
struct SettingsView: View {
    ...
    @AppStorage("beans.themeMode") private var themeModeRaw = YYMusicThemeMode.system.rawValue
    /// 音质等级（借鉴 Kumone）
    @AppStorage("beans.audioQuality") private var audioQualityRaw = YYMusicAudioQuality.exhigh.rawValue
    /// 底栏是否显示文字（关闭后只显示图标）
    ...
    var body: some View {
        ...
        .fileExporter(
            ...
            document: backupDoc,
            contentType: .json,
            defaultFilename: "YYMusic设置备份-\(Self.backupDateString())"
        ) { result in
            switch result {
            case .success:
        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 14) {
                Picker("主题模式", selection: $themeModeRaw) {
                    ForEach(YYMusicThemeMode.allCases) { mode in
                ...
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    Picker("全局 UI 样式", selection: Binding(
                        ...
                        ForEach(YYMusicUIStyle.allCases, id: \.self) { style in
                ...
                HStack(spacing: 12) {
                    ...
                    VStack(alignment: .leading, spacing: 2) {
                        Text("播放设置")
                            .font(YYMusicFont.appFont(15))
                            .foregroundStyle(Color.beansLabel)
                        Text("\(YYMusicAudioQuality(rawValue: audioQualityRaw)?.displayName ?? "高品质") · \(enableBuiltInSources ? "内置音源已开" : "内置音源已关")")
                            .font(YYMusicFont.appFont(11))
                            .foregroundStyle(Color.beansComment)
                            .lineLimit(1)
                    }
            ...
            VStack(spacing: 14) {
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    Picker("音质", selection: $audioQualityRaw) {
                        ForEach(YYMusicAudioQuality.allCases) { q in
    ...
    private func backupActionButton(icon: String, title: String, action: @escaping () -> Void) -> some View {
        Button(action: action) {
            ...
            .padding(.vertical, 12)
            .background {
                YYMusicGlass(shape: RoundedRectangle(cornerRadius: 14, style: .continuous))
            }
        }
    ...
    private func exportBackup(includeAccounts: Bool, includeWallpapers: Bool) {
        ...
        backupMessage = nil
        YYMusicLogger.shared.log("导出配置备份（\(payload.count) 项，账号=\(includeAccounts ? "包含" : "排除") 壁纸=\(includeWallpapers ? "包含" : "排除")）", level: .info)
    ...
    private func applyRestore(_ json: [String: Any]?) {
        ...
        YYMusicLogger.shared.log("恢复配置备份：\(count) 项设置", level: .info)
        if count > 0 {
            YYMusicHaptics.success()
    ...
    private func resetAllSettingsKeepingAccounts() {
        ...
        YYMusicLogger.shared.log("重置所有设置：移除 \(removed) 项，保留登录信息", level: .info)
    }
    ...
    private func logActionButton(icon: String, title: String, action: @escaping () -> Void) -> some View {
        Button(action: action) {
            ...
            .padding(.vertical, 11)
            .background {
                YYMusicGlass(shape: RoundedRectangle(cornerRadius: 13, style: .continuous))
            }
        }
        .buttonStyle(GlassPressButtonStyle(scale: 0.95))
    }
    ...
    private var footerNote: some View {
        VStack(spacing: 6) {
            Text("YY Music · 仅供学习交流，纯 AI 实现此应用")
                .font(YYMusicFont.appFont(11))
                .foregroundStyle(Color.beansComment.opacity(0.7))
