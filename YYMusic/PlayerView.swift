...
struct PlayerView: View {
    ...
    /// 播放器顶部与底部控制按钮的统一样式
    @AppStorage("beans.playerButtonStyle") private var playerButtonStyleRaw = YYMusicPlayerButtonStyle.glass.rawValue
    ...
    private var playerButtonStyle: YYMusicPlayerButtonStyle {
        YYMusicPlayerButtonStyle(rawValue: playerButtonStyleRaw) ?? .glass
    }
    ...
    private var lyricPlayerBackgroundLayer: some View {
        if let image = YYMusicImageFileCache.image(at: lyricBackgroundImagePath) {
                ...
                ForEach(rateOptions, id: \.self) { option in
                    Button {
                        ...
                        Text(String(format: "%.2gx", option))
                            .font(YYMusicFont.appFont(10, .semibold))
    ...
    private func albumPanel(geo: GeometryProxy) -> some View {
        ...
        return VStack(spacing: 16) {
            ...
            Button {
                ...
                ZStack {
                    ...
                    ZStack {
                            ...
                            // 液态玻璃托盘（圆形模式用圆形托盘）
                            if circularCover {
                                YYMusicGlass(shape: Circle())
                                    .frame(width: size * 1.10, height: size * 1.10)
                                    .shadow(color: .black.opacity(0.28), radius: 26, y: 12)
                                    .offset(y: 0)
                            } else {
                                YYMusicGlass(shape: RoundedRectangle(cornerRadius: min(30, size * 0.10), style: .continuous))
                                    .frame(width: size * 1.10, height: size * 1.10)
                                    .shadow(color: .black.opacity(0.28), radius: 26, y: 12)
                                    .offset(y: 0)
                            }
            ...
            VStack(spacing: 6) {
                HStack(spacing: 8) {
                    Text(song?.name ?? "未在播放")
                        .font(YYMusicFont.appFont(22, .bold))
                        .foregroundStyle(palette.text)
                        .lineLimit(2)
        ...
        return VStack(spacing: 3) {
            if rows.isEmpty {
                ...
                ForEach(Array(rows.enumerated()), id: \.offset) { index, item in
                    HStack(spacing: 6) {
                        Text(item.isCurrent ? "●" : "·")
                            .font(YYMusicFont.appFont(8))
                            .foregroundStyle(item.isCurrent ? palette.accent : palette.secondary.opacity(0.5))
                        Text(item.text)
                            .font(YYMusicFont.appFont(12, item.isCurrent ? .semibold : .regular))
            ...
            VStack(spacing: 0) {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    HStack(spacing: 5) {
                        ...
                        if song?.isVIP == true {
                            Text("VIP")
                                .font(YYMusicFont.appFont(8, .bold))
                                .foregroundStyle(.white)
    ...
    private var placeholderView: some View {
        VStack(spacing: 14) {
            Image(systemName: "waveform")
                ...
                .foregroundStyle(palette.secondary)
            Text("暂无播放内容")
                .font(YYMusicFont.appFont(16, .medium))
                .foregroundStyle(palette.text)
            Text("返回选择一首歌曲即可开始播放")
        ...
        VStack(spacing: 1) {
            ...
            HStack(spacing: 6) {
                ...
                Text(beansTimeString(clock.progress))
                    .font(YYMusicFont.appFont(10, .regular, .monospaced))
                    .foregroundStyle(palette.secondary)
    ...
    private var layoutToolbar: some View {
        VStack(spacing: 10) {
            HStack {
                Text("自定义布局")
                    .font(YYMusicFont.appFont(15, .bold))
                Spacer()
                Button {
                    YYMusicHaptics.select()
    ...
    private func layoutSlider(_ title: String, value: Binding<CGFloat>, range: ClosedRange<CGFloat>, step: CGFloat = 1, format: String = "%.0f") -> some View {
        HStack(spacing: 10) {
            ...
            Text(String(format: format, value.wrappedValue))
                .font(YYMusicFont.appFont(11, .regular, .monospaced))
                .foregroundStyle(palette.secondary)
    ...
    private func downloadCurrent(_ quality: DownloadQuality) async {
        ...
        case .success(let downloadResult):
            YYMusicLogger.shared.log("下载完成，弹出分享：\(song.name)（\(quality.rawValue)\(downloadResult.downgraded ? "，已自动降级" : "")）", level: .info)
            sharedFileURL = downloadResult.url
            shareFile = ShareFileItem(url: downloadResult.url)
        case .failure(let error):
            YYMusicLogger.shared.log("下载失败：\(song.name) - \(error.localizedDescription)", level: .error)
            ...
            ZStack(alignment: .leading) {
                ...
                if scrubbing {
                    Text(beansTimeString(scrubValue))
                        .font(YYMusicFont.appFont(11, .semibold, .monospaced))
                        .foregroundStyle(.white)
...
struct LyricsSection: View {
    ...
    private func lyricRow(index: Int, line: LyricLine) -> some View {
        ...
        let lineFont: Font = YYMusicFont.appFont(size)
        ...
        return VStack(spacing: 3) {
            ...
            if let translationText, !translationText.isEmpty {
                Text(translationText)
                    .font(YYMusicFont.appFont(size * 0.68, .regular))
...
struct PlayerSettingsSheet: View {
        ...
        Button {
            ...
            VStack(spacing: 5) {
                RoundedRectangle(cornerRadius: 8, style: .continuous)
                    ...
                    .frame(height: 26)
                Text(preset.name)
                    .font(YYMusicFont.appFont(10, .medium))
        ...
        VStack(alignment: .leading, spacing: 9) {
            if let isExpanded {
                Button {
                    ...
                    HStack(spacing: 10) {
                        Text(title)
                            .font(YYMusicFont.appFont(13, .bold))
                            .foregroundStyle(Color.beansLabel)
                        Spacer()
                        Image(systemName: "chevron.down")
                            .font(.system(size: 12, weight: .semibold))
    ...
    private var playerButtonStyleSelector: some View {
        VStack(alignment: .leading, spacing: 9) {
            ...
            ForEach(YYMusicPlayerButtonStyle.allCases) { style in
...
private struct PlayerSettingsLiquidGlass<S: Shape>: View {
    @AppStorage("beans.uiStyle") private var uiStyleRaw = YYMusicUIStyle.liquid.rawValue
    let shape: S

    private var uiStyle: YYMusicUIStyle {
        YYMusicUIStyle(rawValue: uiStyleRaw) ?? .liquid
    }
