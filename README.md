...
```
YYMusic/
├── YYMusicApp.swift                应用入口（首次使用引导页 + 免责确认）
...
xcodegen generate
xcodebuild -project YYMusic.xcodeproj -scheme YYMusic -configuration Release \
  -sdk iphoneos -derivedDataPath build \
  CODE_SIGNING_ALLOWED=NO CODE_SIGNING_REQUIRED=NO CODE_SIGN_IDENTITY="" build
mkdir -p Payload
cp -R build/Build/Products/Release-iphoneos/YYMusic.app Payload/
ditto -c -k --sequesterRsrc --keepParent Payload YYMusic-unsigned.ipa
```
